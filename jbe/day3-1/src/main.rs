...
// the length of the compartment. This uses two partern-defeating quicksorts which are only O(n *
// log(n)) worst case, followed by a simple traversal in O(n). This means this should be worst case
// O(n * log(n) + n). In terms of memory, the sorting is done in place, which means we only
