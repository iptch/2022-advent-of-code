...
def play_round(monkeys, magic_factor_to_manage_my_worry):
    for monkey in monkeys:
        for _ in range(len(monkey.items)):
            monkey.inspection_counter += 1
            old = monkey.items.pop(0)
            new = eval(monkey.operation)
            new = new % magic_factor_to_manage_my_worry
...
def solve(lines):
    ...
    magic_factor_to_manage_my_worry = math.prod([monkey.test_divider for monkey in monkeys])
    for i in range(10000):
        print(f'play round {str(i)}...')
        monkeys = play_round(monkeys, magic_factor_to_manage_my_worry)
