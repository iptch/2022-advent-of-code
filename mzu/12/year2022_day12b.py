...
def solve(lines):
    ...
    start_positions = find_all_start_positions(height_map)
    best_path_length = int_inf
    counter = 1
    for start_row_idx, start_col_idx in start_positions:
        print(f'Evaluating start position {str(counter)} from {str(len(start_positions))}.')
        ...
        try:
            ...
            if len(optimal_path) - 1 <= best_path_length:
                best_path_length = len(optimal_path) - 1
        except TypeError:
            pass
        counter += 1
