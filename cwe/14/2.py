...
def draw_cave(file_path):
    ...
    # add empty row and solid floor
    cave = np.vstack([cave, np.zeros(1000)])
    cave = np.vstack([cave, np.full(1000, 7)])
    return cave
    ...
    while True:
        ...
        while moving:
            if cave[sand[0] + 1, sand[1]] == 0:
                ...
                moving = False
        elements += 1
    print(f'Filled hole after {elements} elements')
    print('end')
